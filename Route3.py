...
def Route3():
    ...
    rightArm.run_time(400, 300, then=Stop.BRAKE, wait=False) # Σηκώνεται ψηλά ο μοχλός
    MoveSteering_Seconds(-50, 0, 300) # Μικρή όπισθεν, κολλάει τοίχο

    #### Αποχώρηση από τη Βάση ####
    MoveStraight_Distance(500,600,270,True,True,Stop.BRAKE) # Φεύγει από τη Βάση
    PointTurn_Angle(700, 700, -46, True, Stop.BRAKE) # Στροφή αριστερά 46 μοίρες
    MoveStraight_Distance(500,600,175,True,True,Stop.BRAKE) # Ευθεία, είναι μπροστά από το "Τι είναι προς Πώληση"
    ...
    rightArm.run_angle(1000, 100, then=Stop.BRAKE, wait=True) # Επαναφορά μοχλού
    
    MoveStraight_Distance(300,600,40,True,True,Stop.BRAKE) # Λίγο Ευθεία, μπαίνει ο μοχλός εντός της Τέντας
    ...
    MoveStraight_Distance(150,600,-100,True,True,Stop.BRAKE) # Όπισθεν, απομάκρυνση από Τέντα
